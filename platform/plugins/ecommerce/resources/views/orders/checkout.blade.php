...
<script>
    this.$body = $('body');
     let _self = this;
    _self.$body.on('click', '#address_state', event => {
        let stateID = $('#address_state').val();
        if(stateID){
        $.ajax({
           type:"GET",
           url:"{{url('checkout')}}/{{$token}}/city/"+stateID,
           dataType: 'JSON',
           success:function(res){
            if(res){
                $("#address_city").empty();
                $("#address_city").append('<option>---City---</option>');
                $.each(res,function(key,city){
                    $("#address_city").append('<option value="'+city.name+'">'+city.name+'</option>');
                });
            }else{
               $("#address_city").empty();
            }
